...
@DB_View(name = "purchase_summary", tables = {
		@ViewTable(typeName = PostgresCustomerTable.class, asName = "c", columns = {
		...
		@ViewTable(typeName = PostgresPurchaseTable.class, join = ViewTable.Type.INNER, asName = "p", on = "c.id = p.customer_id", columns = {
				@ViewColumn(name = "id", asName = "purchase_id"), @ViewColumn(name = "amount", asName = "amount") }) })
public abstract class PostgresPurchaseSummaryView extends DeferredDataBaseView<PurchaseSummaryData>
		implements PurchaseSummaryViewAccess {

	public PostgresPurchaseSummaryView(@Qualifier("postgresDemo") final DataBase dataBase) {
		super(dataBase);
	}
}
